...
namespace cuda
{
__host__ uint32_t jamcrcv2(const void* data, uint64_t& length, uint32_t& previousCrc32);

__host__ void launch_kernelv2(std::vector<uint32_t>& jamcrc_results,
                              std::vector<uint64_t>& index_results,
                              const uint64_t& min_range,
                              const uint64_t& max_range,
                              const uint64_t& cuda_block_size);

__host__ void launch_kernelv2(size_t grid,
                              size_t threads,
                              cudaStream_t& stream,
                              uint32_t* crc_result,
...
__host__ void launch_kernelv2(dim3& grid,
                              dim3& threads,
                              cudaStream_t& stream,
                              uint32_t* crc_result,
                              uint64_t* index_result,
